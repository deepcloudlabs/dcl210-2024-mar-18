...
public class MostCpuConsumingProcess {
	...
	public static void main(String[] args) {
		...
		Optional<ProcessHandle.Info> processInfo = ProcessHandle.allProcesses().map(p -> p.info())
				.filter(info -> info.totalCpuDuration().isPresent()).max((p, q) -> p.totalCpuDuration()
						.orElse(Duration.ZERO).compareTo(q.totalCpuDuration().orElse(Duration.ZERO)));
